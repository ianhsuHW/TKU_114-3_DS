public class Q11_JobSorter {
    ...
    private static void merge(
        ...
        while (leftIndex <= mid && rightIndex <= right) {
            if (jobs[leftIndex].getPriority() >=
                jobs[rightIndex].getPriority()) {
                temp[tempIndex] = jobs[leftIndex];
                leftIndex++;
            } else {
                temp[tempIndex] = jobs[rightIndex];
                rightIndex++;
            }
            tempIndex++;
        }
    ...
    public static Q11_Job findFirstByOwner(
        ...
        String target = owner.trim();

        for (Q11_Job job : jobs) {
            if (job != null && job.getOwner() != null &&
                job.getOwner().trim().equalsIgnoreCase(target)) {
                return job;
            }
        }
        return null;
